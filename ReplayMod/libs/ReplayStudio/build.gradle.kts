...
dependencies {
    viaVersion("com.github.replaymod.viaversion:viaversion-common:aec926bbc")
...
tasks.register<JavaExec>("extractInventory") {
    group = "replaystudio"
    description = "Extract inventory data from an MCPR file"
    classpath = sourceSets.main.get().runtimeClasspath
    mainClass.set("com.replaymod.replaystudio.tools.InventoryExtractor")
    val path = project.findProperty("mcpr")?.toString()
    if (path != null) {
        args(path)
    }
    val timestamp = project.findProperty("timestamp")?.toString()
    if (timestamp != null) {
        args(timestamp)
    }
...
tasks.register<Jar>("inventoryToolJar") {
    group = "replaystudio"
    description = "Build standalone inventory tool jar"
    from(sourceSets.main.get().output)
    from(configurations.runtimeClasspath.get().map { if (it.isDirectory) it else zipTree(it) })
    duplicatesStrategy = DuplicatesStrategy.EXCLUDE
    archiveFileName.set("inventory-tool.jar")
    manifest {
        attributes(mapOf("Main-Class" to "com.replaymod.replaystudio.tools.InventoryExtractor"))
    }
}

tasks.register<Jar>("inventoryToolGuiJar") {
    group = "replaystudio"
    description = "Build standalone GUI inventory tool jar"
    from(sourceSets.main.get().output)
    from(configurations.runtimeClasspath.get().map { if (it.isDirectory) it else zipTree(it) })
    duplicatesStrategy = DuplicatesStrategy.EXCLUDE
    archiveFileName.set("inventory-tool-gui.jar")
    manifest {
